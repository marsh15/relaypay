...
export function DisputeActions({ environmentId, caseId, csrfToken, latestDraft, latestPackage }: { environmentId:string; caseId:string; csrfToken:string; latestDraft:Draft | null; latestPackage:Package | null }) {
  ...
  return <section className="resource-panel" aria-labelledby="case-actions-title"><div className="resource-heading"><div><p className="section-kicker">Controlled actions</p><h2 id="case-actions-title">Review and submit</h2></div></div>
    <div className="dispute-actions">
      ...
      {latestDraft ? <form onSubmit={(event) => {event.preventDefault(); const data = new FormData(event.currentTarget); void command(`/disputes/${caseId}/drafts`, { classification: latestDraft.classification, confidence: latestDraft.confidence, missingEvidence: latestDraft.missingEvidence, responseText: String(data.get("responseText")), citations: latestDraft.selectedEvidence });}}><label htmlFor="responseText">Analyst response</label><textarea id="responseText" name="responseText" rows={8} defaultValue={latestDraft.responseText} required maxLength={20000}/><p className="supporting-copy">Saving creates a new version and invalidates every prior approval.</p><button className="button button-secondary" disabled={busy} type="submit">Save immutable revision</button></form> : null}
