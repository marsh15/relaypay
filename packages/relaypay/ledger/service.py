...
def _account(session: Session, organisation_id: uuid.UUID, code: str) -> LedgerAccount:
...
def _post(
    ...
    *,
    ...
    debit = _account(session, organisation_id, debit_code)
    credit = _account(session, organisation_id, credit_code)
    journal = Journal(
        id=new_uuid(),
